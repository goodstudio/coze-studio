...
func New(ctx context.Context, s *schema.NodeSchema, inner compose.Runnable[map[string]any,
	map[string]any], sc *schema.WorkflowSchema, deps *dependencyInfo) (_ *Node, err error) {
	defer func() {
	...
	if m := entity.NodeMetaByNodeType(s.Type); m != nil && m.InputSourceAware {
		if fullSources, err = GetFullSources(s, sc.GetAllNodes(), deps); err != nil {
			return nil, err
		}
